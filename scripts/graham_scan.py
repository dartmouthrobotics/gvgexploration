...
def incremental_farthest_search(points, k):
    ...
    solution_set = []
    solution_set.append(remaining_points.pop(randint(0, len(remaining_points) - 1)))
    for _ in range(k-1):
...
def graham_scan(points, k, show_progress=False):
    ...
    frontier_points=incremental_farthest_search(hull,k)
    return hull,frontier_points
